    ...
    Process { 
        ...
        if ($InstallBasedOnOfficeBitness) {
            ...
            switch ($obj.OSIs64Bit) {
                $True {start-process -FilePath "$ENV:SystemRoot\System32\msiexec.exe" -ArgumentList "/i ""$workingDir\Your64-BitMSI"" /qn" -NoNewWindow -Wait}
                default {start-process -FilePath "$ENV:SystemRoot\System32\msiexec.exe" -ArgumentList "/i ""$workingDir\Your32BitMSI"" /qn" -NoNewWindow -Wait}
            }
        }
